...
- 若答案页是图片，可使用 `Script/paddle_ocr_answers.py` 调 PaddleOCR 提取侧车答案：
  ```bash
  PADDLE_OCR_TOKEN=your_token_here \
  conda run -n ielts python Script/paddle_ocr_answers.py \
    "path/to/file.pdf" \
    --dump-jsonl /tmp/file_ocr.jsonl
  ```
  成功后会在 PDF 同目录生成 `xxx.answers.json`，现有 HTML/状态服务器可直接读取。
- 若要批量补答案侧车，可直接运行：
  ```bash
  PADDLE_OCR_TOKEN=your_token_here \
